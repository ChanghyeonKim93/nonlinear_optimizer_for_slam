...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::SolveDouble(
...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::SolveFloatMatrix(
    const Options& options, const std::vector<Correspondence>& correspondences,
    Pose* pose) {
...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::SolveFloatMatrixAligned(
    const Options& options, const std::vector<Correspondence>& correspondences,
    Pose* pose) {
  using Vec3f = Eigen::Vector3f;
  using Mat3x3f = Eigen::Matrix3f;
  ...
  AlignedBufferVarious abuf(correspondences.size());
...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::SolveDoubleMatrix(
    const Options& options, const std::vector<Correspondence>& correspondences,
    Pose* pose) {
  constexpr double min_lambda = 1e-6;
  constexpr double max_lambda = 1e-2;
...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::Solve(
    const Options& options, const std::vector<Correspondence>& correspondences,
    Pose* pose) {
  using Vec3f = Eigen::Vector3f;
  using Mat3x3f = Eigen::Matrix3f;
...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::SolveFloatAligned(
    const Options& options, const std::vector<Correspondence>& correspondences,
    Pose* pose) {
  using Vec3f = Eigen::Vector3f;
  using Mat3x3f = Eigen::Matrix3f;
  ...
  AlignedBufferVarious buf(correspondences.size());
  for (size_t index = 0; index < correspondences.size(); ++index) {
    const auto& corr = correspondences.at(index);
...
bool MahalanobisDistanceMinimizerAnalyticSIMDVarious::
    SolveFloatIntrinsicAligned(
  ...
  AlignedBufferVarious aligned_buf(correspondences.size());
  for (size_t index = 0; index < correspondences.size(); ++index) {
    const auto& corr = correspondences.at(index);
...
void MahalanobisDistanceMinimizerAnalyticSIMDVarious::ReflectHessian(
    Mat6x6* hessian) {
  auto& H = *hessian;
